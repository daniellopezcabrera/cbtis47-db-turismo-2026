...
| **Acceptance Criteria (Flights):** <br> • **Search:** Allow input of origin, destination, dates, and passengers with results including updated schedules, airline, duration, and current prices. <br> • **Selection:** Display a detailed summary (flight, date, price) and require confirmation before proceeding to payment. <br> • **Payment & Issuance:** Offer secure payment methods (Credit/Debit) and generate a digital receipt with a reservation code after payment. <br> • **Seat Selection:** Enable choosing a specific seat showing real-time availability. |
| **Acceptance Criteria (Tram):** <br> • **Routes:** Display a list of available trams, schedules, routes, and full itineraries with scheduled stops. <br> • **Availability:** Block reservations if the unit's maximum capacity is reached and show a clear "not available" message if there is no vacancy. <br> • **Booking:** Register the reservation in the database and generate a digital ticket with the reserved seat for immediate download. |
| **Validations & Integration:** <br> • **Integrated Cart:** Manage both flight and tram services in a single shopping cart, allowing modifications or removals before confirming the purchase. <br> • **Date Sync:** The system must verify that tram dates align with the flight stay; otherwise, show an error message or adjustment suggestion. <br> • **Admin Management:** Allow administrators to register new units (ID and capacity) and manage schedules without overbooking conflicts. |
| **Status Inquiry:** <br> • Users must be able to access their profile to see the updated status (gate for flights, meeting point for trams) or a "no modifications" message if there are no changes. |

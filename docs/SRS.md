...
**Flygth With You** is a web-based reservation management system for a tourism agency. It allows users to register, log in, search and book flights or tourist trolleybus trips, process payments, and download their ticket as a PDF file as proof of their confirmed reservation.
...
- Flight search, reservation, and payment.
- Tourist trolleybus search, reservation, and payment.
- Single-download PDF ticket generation.
- Flight and route management by the administrator.
- Assigned flight lookup for pilots and co-pilots.
- In-flight incident recording by flight attendants.
- Daily trip lookup for trolleybus drivers.
...
- Deployment to a public production server.

---

## 2. Technology Stack

The system is built exclusively with standard client-side web technologies, with no additional backend frameworks, in compliance with NFR-17.

| Component | Technology / Version | Purpose in the project |
|---|---|---|
| Page structure | HTML5 (Living Standard) | Semantic markup for all views of the system. |
| Visual presentation | CSS3 (Living Standard) | Styles, responsive design, animations, and seat map rendering. |
| Client-side logic | JavaScript ES2025 (ECMAScript 16th edition) | Event handling, validations, 10-minute timer, Supabase integration, and PDF generation. |
| Database & authentication | Supabase · supabase-js v2.107.0 | PostgreSQL as the main database and Supabase Auth for session and credential management. |
| PDF generation | jsPDF v4.2.1 | Client-side ticket generation and download directly from the browser, with no intermediate server. |
| CSS framework | Tailwind CSS v3.4 | Utility classes for building the interface quickly and consistently, without redundant custom CSS. Fully compatible with the project's plain HTML + JS stack. |
| Version control | Git + GitHub | Change history, team collaboration, and source code backup (AG-08, AG-09). |

### 2.1 CSS Framework Justification — Tailwind CSS v3

The project prohibits additional backend frameworks (NFR-17), but does not restrict the use of client-side tooling. Tailwind CSS v3 is selected for the following reasons:

- It is a purely frontend tool (a CSS file). It requires no server or mandatory build process; it can be integrated via CDN in any HTML file.
- It maintains the visual consistency required by UX-01 without duplicating CSS rules across multiple files.
- Version 3.x is broadly compatible with modern Chromium-based browsers (NFR-23).
- It facilitates the responsive design required by UX-02 through its breakpoint prefixes (`sm:`, `md:`, `lg:`).

### 2.2 Loading Dependencies (no package manager)

The project does not use a package manager (npm/yarn). All external libraries are loaded directly via `<script>` tags inside the `<head>` of each HTML file:
...
**NFR-24.** The system will not allow flights or trolleybuses to be registered on past dates or times; it will display an error message.

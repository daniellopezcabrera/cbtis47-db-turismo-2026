...
Flight | price | DECIMAL(10,2) NOT NULL CHECK (price > 0) | Flight ticket price (must be positive)
...
Booking | id_flight | INT (FK) NOT NULL | Flight associated with the booking (references Flight.id_flight)
Booking | date | DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP | Date and time when booking was created
...
Ticket | id_booking | INT (FK) NOT NULL | Booking associated with the ticket (references Booking.id_booking)
Ticket | qr_code | VARCHAR(255) UNIQUE NOT NULL | Unique QR code to validate the ticket
...
Trolley | plate_number | VARCHAR(20) UNIQUE NOT NULL | Vehicle license plate number
Trolley | id_model | INT (FK) NOT NULL | Model assigned to the trolley (references Trolley_model.id_model)
...
Trolley_stop | name | VARCHAR(100) NOT NULL | Name of the stop
Trolley_stop | coordinates | VARCHAR(100) NOT NULL | Geographic coordinates (latitude, longitude)

Route_stop | id_route | INT (PK, FK) NOT NULL | Route that includes the stop (references Route.id_route)
Route_stop | id_stop | INT (PK, FK) NOT NULL | Stop included in the route (references Trolley_stop.id_stop)

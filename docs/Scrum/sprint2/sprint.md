...
> **Previous sprint:** See [`sprint-01.md`](./sprint-01.md) for the initial backlog, database standards, and design decisions established in Sprint 1.
...
7. [Agile Requirements](#7-agile-requirements)
...
Used to generate and download the reservation ticket as a PDF file directly from the browser.
...
**NFR-08.** The 10-minute seat hold timer must be enforced server-side through database timestamps (the `selected_at` and `expires_at` fields in `BOOKING_SEAT`), not only in the user's browser.
...
Feature: PDF Ticket Download
  ...
  Scenario: Successful first-time download
    ...
    Then the system generates a PDF using jsPDF
...
erDiagram
    ...
    PAYMENT {
        ...
        varchar card_last_four "last 4 digits only — never full card data"
    }
```
